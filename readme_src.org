...
| =Psi4Pot= | =-Dwith_psi4=true= | custom / =third_party/libpsi4-cpp= | BLYP via =libpsi4_engine= (=dlopen=; no Python) |
| =NWChemPot= | frontend always; engine =-Dwith_nwchem=true= =-Dnwchem_root=$NWCHEM_TOP= | NWChem install | QM via =libnwchem_engine= (=dlopen=); stub ABI without NWChem |
