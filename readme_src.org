...
| =NWChemPot= | frontend always; engine =libnwchemc= built by the split =nwchemc= project | NWChem source/install tree | QM via =libnwchemc= (=dlopen= in-process C ABI; no CLI) |
