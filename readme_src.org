...
| =NWChemPot= | frontend always; embed engine =-Dwith_nwchem=true= =-Dnwchem_root=$NWCHEM_TOP= | NWChem source/install tree | QM via =libnwchem_engine= (=dlopen= in-process C ABI; no CLI) |
