...
| =ExprPot= | in-process | Enable with =-Dwith_expr=true=. Vendored OpenMM Lepton (no pixi muparser). Named =Potential= children; construct-time fail-closed names. No =PotentialConfig.expr=. Wire tree is =ExprParams= / =PotSpec=; C++ constructor is the first-slice API. |
