...
| =D3Pot= | =D3= | Enable with =-Dwith_dftd3=true=; use pixi env =dftd3= or =dftd=. BJ/zero damping, functional key, explicit ATM flag. Goldens: =meson test --suite dftd= |
| =D4Pot= | =D4= | Enable with =-Dwith_dftd4=true=; use pixi env =dftd4= or =dftd=. Functional key, charge, explicit ATM/many-body flag. Goldens: =meson test --suite dftd= |
