//! Lockstep semver surfaces for the rgpot monorepo (meson / CMake / cargo / towncrier / pixi / pyproject).
...
pub fn assert_lockstep(root: &Path, expected: Option<&str>, require_changelog: bool) -> Result<()> {
    ...
    let pyproject_v = read_pyproject_version(root)?;
    ...
    println!("  pyproject.toml   = {pyproject_v}");
