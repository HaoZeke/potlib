...
Energy/grad tests =SKIP= unless =libnwchemc= or the transitional
=libnwchem_engine= is on the library path (=NWCHEMC_LIBRARY=,
=RGPOT_NWCHEMC_ENGINE=, or =RGPOT_NWCHEM_ENGINE=). The message ABI test uses a
fake =nwchemc_*= shared library and verifies rgpot passes serialized
=NWChemParams= bytes through the =dlopen= boundary.
