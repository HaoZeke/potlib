...
=meson.build=, =CMakeLists.txt= (=project(VERSION ...)=),
=rgpot-core/ Cargo.toml=, =towncrier.toml=, and =pixi.toml= (workspace key only
...
- Prefer *signed tags* when cutting locally: =git tag -s vX.Y.Z= (or ensure
  =cog=
produces signed tags if configured).
GHA does not replace maintainer GPG.

*C++ distribution note*:
tag/source is the C++ contract; =release.yml= runs a minimal =meson install= smoke. Optional backends (xtb/tblite/metatomic)
are validated on PRs via orchestrator potentials jobs, not re-run on every tag.
...
(orchestrator =towncrier= runs on all PRs; fragments under
=CppCore/=, =rgpot- core/=, meson, =CMakeLists.txt=, =pixi.toml=, release
scripts, etc.).
...
relevant).
2. Confirm fragments exist for everything since the last tag (=uvx towncrier
   build
--draft=).
3. Local sanity:
...
4. Dry-run bump (optional; also GHA =workflow_dispatch= on
   =release-prepare.yml=):
...
5. Cut the release *locally* (needs push rights; does *not* publish to crates.io
   by
...
Hooks (=cog.toml= =pre_bump_hooks=):
   ...
   - =cargo run -q -p potctl -- release sync {{version}}= — lockstep write
     (meson,
CMake, towncrier, Cargo, pixi workspace)
   - =uvx towncrier build --yes --version {{version}}= — update =CHANGELOG.md=,
     clear
fragments
...
6. Tag push triggers =release.yml= (staged, not one fat job):
   ...
   - =tools= job: =setup-ci-tools= builds =potctl= (rust-cache =shared-key=
     =potctl-
...
Across jobs, binaries travel via artifacts;
=cargo=/deps hit =Swatinem/rust- cache= on tool and =rgpot-core= runners so the
next tag cut is cheap when sources are unchanged.
...
stamp, and pixi workspace version share one semver (optionally with
...
3. On =main=: =cog bump --version 1.2.0= (or =--auto= if you want cog to
   decide).
This runs =potctl release sync=, =towncrier build= (so =CHANGELOG.md= gains
=## [1.2.0]= and fragments are cleared), then
=potctl release assert --require- changelog=.
4. =git push origin main --follow-tags= (or push commit then =v1.2.0=
   explicitly).
5. Confirm =Release= workflow: lockstep gate, smoke install, rust tests,
   crates.io,
GH Release body.
