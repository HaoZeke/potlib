...
| =-Dwith_nwchem=true= =-Dnwchem_root=$NWCHEM_TOP= | full NWChem source/install | =libnwchem_engine.so= in-process embed (C ABI, =dlopen=) |

=NWChemPot= frontend is always compiled; it =dlopen=s optional =libnwchem_engine.so=
built only with embed flags (stable C symbols in =nwchem_c_abi.h=). No =nwchem=
CLI/subprocess path. Clone+build NWChem once (=scripts/setup_nwchem_embed.sh=),
then set =RGPOT_NWCHEM_ENGINE=. See =CppCore/rgpot/NWChemPot/README.md=.
