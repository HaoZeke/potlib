...
| =-Dwith_psi4=true= | (custom / third_party) | =Psi4Pot= frontend + =libpsi4_engine= (runtime =dlopen=) |
| =-Dwith_nwchem=true= =-Dnwchem_root=$NWCHEM_TOP= | (NWChem install) | optional =libnwchem_engine=; =NWChemPot= frontend always built |

=NWChemPot= is always compiled into the main library (frontend only). Enable
=-Dwith_nwchem=true= with a non-empty =-Dnwchem_root=...= to build the optional
shared engine that embeds NWChem via =nwchem_embed.F=. At runtime set
=RGPOT_NWCHEM_ENGINE= / =NWCHEM_TOP= as needed. See =CppCore/rgpot/NWChemPot/README.md=.
