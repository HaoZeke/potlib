...
has no =-Dwith_nwchem= option.
No =nwchem= CLI/subprocess path.
Build =libnwchemc.so= from the =nwchemc= project, then set =NWCHEMC_LIBRARY= or
...
and =dlopen=s =libcpmdc.so= from =https://github.com/OmniPotentRPC/cpmdc=. rgpot
builds no in-process OpenCPMD embed of its own and has no =-Dwith_cpmd= option.
