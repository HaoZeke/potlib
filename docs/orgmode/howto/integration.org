...
*** NWChem backend over RPC

When the server is started with =potserv <port> NWChem=, clients should call
=configure= with =PotentialConfig.nwchem= (=NWChemParams=) before =calculate=.
Every option is mapped server-side to =rgpot::NWChemConfig= and (for compute
fields) the embed C ABI:

| =NWChemParams= (capnp) | =NWChemConfig= (C++) | embed / runtime |
|------------------------+----------------------+-----------------|
| =basis= | =basis= | basis set |
| =theory= | =theory= | =scf= / =dft= / =blyp= / =b3lyp= / … |
| =scfType= | =scf_type= | =rhf=/=uhf= or DFT =xc= (=blyp=, …) |
| =charge= | =charge= | molecular charge |
| =multiplicity= | =multiplicity= | =2S+1= |
| =enginePath= | =engine_path= | =dlopen= =libnwchem_engine.so= |
| =nwchemRoot= | =nwchem_root= | =NWCHEM_TOP= for embed data/libs |

Example BLYP: =theory="dft"=, =scfType="blyp"= (or =theory="blyp"=). Python:
=tests/nwchem_params.py= (=make_nwchem_params= / =configure_nwchem=). Without a
built embed =.so=, =configure= may return =ok=false=; =calculate= fails until
=RGPOT_NWCHEM_ENGINE= points at a real engine.
