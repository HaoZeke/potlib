...
With =potserv <port> CPMD=, pass =configure= with the =cpmd= arm set. Payload
fields include method-level defaults (=functional=, =cutOffRy=, =charge=,
=multiplicity=, =task=), frontend hints (=enginePath=, =cpmdRoot=), runtime file
placement (=scratchDir=, =permanentDir=), raw =inputBlocks=, and structured
=inputSections= for =SYSTEM=, =CPMD=, =DFT=, =ATOMS=, =SET=, and raw sections.

Python: =tests/cpmd_params.py= builds =PotentialConfig= + =cpmd=. The helper
uses =CPMD_ROOT= for =cpmdRoot= and =CPMDC_LIBRARY= / =RGPOT_CPMDC_ENGINE= /
=RGPOT_CPMD_ENGINE= for =enginePath= when explicit values are not supplied.
...
for every atomic number present in a step. Without an =atoms= section, built-in
BLYP pseudopotential defaults cover H and O only. The same =atoms= section also
accepts structured =directives= for ATOMS keywords that are not
pseudopotential entries.
The integration script can exercise configure-only RPC plumbing without a CPMD
engine:
