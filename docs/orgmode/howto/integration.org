...
When the server is started with =potserv <port> NWChem=, clients may call
=configure= with =PotentialConfig.nwchem= (=NWChemParams=: basis, theory,
=scfType=, charge, multiplicity, optional =enginePath= / =nwchemRoot=) before
=calculate=. Without =libnwchem_engine= on the server host, =configure= may
return =ok=false= while the RPC still completes; =calculate= fails until an
engine is provided. Python helpers: =tests/nwchem_params.py=.
