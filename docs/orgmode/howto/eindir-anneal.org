#+TITLE: rgpot Potentials as eindir Objectives (and Minimizing Them with anneal)
#+OPTIONS: toc:nil todo:nil num:nil author:nil title:nil
#+STARTUP: content

* rgpot Potentials as eindir Objectives

This guide shows how an rgpot potential becomes an
[[https://github.com/HaoZeke/ eindir][eindir]] objective and how
[[https://github.com/HaoZeke/anneal][anneal]] then minimizes it.
...
eindir defines the objective contract twice, on purpose, with a bridge between
the two halves:
...
- =EindirObjectiveWrapper=, which borrows an =eindir_objective_t= and presents
  it
...
*not* a prebuilt =libeindir_core.a=.
One shared compilation of eindir-core backs both crates, so a downstream Rust
binary that links rgpot-core and anneal- core (both depending on eindir-core)
gets a single eindir-core and avoids the duplicate-symbol / two-Rust-runtime
conflict a static lib would cause.
...
A runnable end-to-end demonstration lives in the anneal repo at
=examples/ rgpot_minimize.rs= (=cargo run --example rgpot_minimize=).
...
** Compatibility validation at the bridge boundary

The bridge validates the producer before the first objective evaluation. The
native =eindir_core_abi_stamp_t= must match ABI major, objective layout, object
size, and alignment exactly; additive ABI and DLPack minor revisions may not
exceed the consumer. Required feature bits must be advertised by the producer.

The semantic =eindir_objective_descriptor_t= additionally carries schema and
producer identity, length and energy units, energy/gradient signs, operations,
DLPack device and dtype, tensor layout, and callback lifetime. Consumers use
=eindir_objective_descriptor_compatible= with required operation bits as a
subset. Empty strings and zero scalar requirements are wildcards. A mismatch
is rejected before evaluation and the bridge exposes the diagnostic through
=eindir_last_error=.

The contract is versioned as =eindir.objective= and the current descriptor
requires contiguous CPU floating-point tensors, synchronous borrowed callback
storage, energy and force operations, and the published tensor-aware layout.
