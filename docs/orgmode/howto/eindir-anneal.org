...
This guide shows how an rgpot potential becomes an [[https://github.com/HaoZeke/
eindir][eindir]] objective and how [[https://github.com/HaoZeke/anneal][anneal]]
then minimizes it.
...
=rgpot_potential_t= embeds =eindir_objective_t= as its *first* member, so
a =rgpot_potential_t*= is an =eindir_objective_t*= at zero cost (the C "is-
a" idiom).
...
A runnable end-to-end demonstration lives in the anneal repo at =examples/
rgpot_minimize.rs= (=cargo run --example rgpot_minimize=).
