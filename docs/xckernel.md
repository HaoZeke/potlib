...
`xck_*_st_o2_p` kernels plus host Coulomb. TDA/RPA stage A stays
double (not the Fock long-double override); LDA wv is
`w * rho * (v2rho2_0 + v2rho2_1)`. Stage B tiles the grid at PySCF
`BLKSIZE` (128), matching `nr_rks_fxc` / `_dot_ao_ao`.
