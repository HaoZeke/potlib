...
// D3Pot and that the explicit ATM/mdb flag and charge change the energy.
...
// Water (Å): O + 2H  — same fixture as D3PotTest.cc
...
TEST_CASE("D4Pot charge flips water energy", "[d4][charge]") {
  AtomMatrix positions(3, 3);
  std::vector<int> atmtypes;
  std::array<std::array<double, 3>, 3> box{};
  fill_water(positions, atmtypes, box);
  ...
  neut.charge = 0.0;
  neut.atm = true;
  rgpot::D4Config cation = neut;
  cation.charge = 1.0;

  rgpot::D4Pot pot0(neut);
