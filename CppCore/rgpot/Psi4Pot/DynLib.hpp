 ...
 *
 * Used by Psi4Pot to resolve libpsi4 + libpsi4_engine at runtime without a
 * build-time link dependency on Psi4.
 */
...
public:
  ...
  void open(const std::string &path) {
    close();
#if defined(_WIN32) || defined(_WIN64)
    // UTF-8 path best-effort via multi-byte API
    handle_ = LoadLibraryA(path.c_str());
    if (!handle_) {
      throw std::runtime_error("LoadLibrary failed: " + path);
    }
#else
    // RTLD_GLOBAL: export libpsi4 symbols for the engine .so to resolve.
