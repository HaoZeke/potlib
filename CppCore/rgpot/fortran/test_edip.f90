...
!> Physics checks for the EDIP gather kernel, run from pure Fortran so a
...
!! a mis-attributed coordination or end-atom term breaks the difference
!! check even where the energy stays right.
program test_edip
   ...
   call edip_energy_forces(positions, cell, par, table, energy, forces, &
                           status, errmsg)
   ...
   call edip_energy_forces(moved, cell, par, table, shifted_energy, forces, &
                           status, errmsg)
   ...
   do k = 1, 12
      ...
      call edip_energy_forces(moved, cell, par, table, e_plus, forces, &
                              status, errmsg)
      moved(comp, atom) = moved(comp, atom) - 2.0_wp*delta
      call edip_energy_forces(moved, cell, par, table, e_minus, forces, &
                              status, errmsg)
   ...
   if (failed) then
      error stop "test_edip: failures above"
   end if
   print *, "test_edip: ok, worst force deviation ", worst
