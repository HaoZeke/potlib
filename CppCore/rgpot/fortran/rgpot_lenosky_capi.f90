...
!> C entry point for the Lenosky silicon potential.
!!
...
module rgpot_lenosky_capi
   ...
   public :: rgpot_lenosky_force

   type(neighbor_table_t), save :: table
   type(lenosky_params_t), save :: params

contains

   !> Evaluate Lenosky forces and energy.
   !!
   ...
   function rgpot_lenosky_force(natoms, positions, cell, forces, energy) &
      result(status) bind(c, name="rgpot_lenosky_force")
      ...
      call lenosky_energy_forces(positions, cell, params, table, energy, &
                                 forces, kernel_status, errmsg)
      if (kernel_status /= 0) then
         call set_error(errmsg)
         status = int(kernel_status, c_int)
      end if
   end function rgpot_lenosky_force

end module rgpot_lenosky_capi
