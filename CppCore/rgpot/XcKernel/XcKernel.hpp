 ...
 * (Fock o1 + fxc o2). TDA/RPA sigma is host algebra around the singlet
 * spin-adapted o2 kernels (`xck_*_st_o2_p`) plus host-owned Coulomb.
...
public:
  ...
  static void aoFromOv(const XcMo &mo, const double *z, double occ, double *dm);

  /// dm = occ * (Cv @ X^T @ Co^T + Co @ Y @ Cv^T).
  static void aoFromXy(const XcMo &mo, const double *x, const double *y,
                       double occ, double *dm);

  /// ov = Co^T @ Vao^T @ Cv (nocc*nvir).
  static void ovFromAo(const XcMo &mo, const double *Vao, double *ov);

  /// (A z)_ia = e_ia z_ia + ovFromAo(v1). v1 is AO (nao*nao).
